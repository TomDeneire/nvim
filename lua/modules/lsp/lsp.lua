...
local linters = { "gofumpt", "stylua", "golangci-lint", "selene" }
...
return {
    ...
    config = function()
        vim.api.nvim_create_autocmd("LspAttach", {
            ...
            callback = function(event)
                ...
                local nmap = function(keys, func, desc, opts)
                    opts = opts or {}
                    local final_opts =
                        vim.tbl_deep_extend("force", { buffer = event.buf, desc = "LSP: " .. desc }, opts)
                ...
                end, "Goto [D]efinition(s)")
                nmap("gd", function()
                    Snacks.picker.lsp_type_definitions()
                end, "Goto Type [D]efinition(s)")
                nmap("gD", function()
                    Snacks.picker.lsp_declarations()
                end, "Goto [D]eclaration(s)")
                nmap("gI", function()
                ...
                nmap("gS", function()
                    Snacks.picker.workspace_symbols()
        ...
        require("mason-tool-installer").setup({
            ensure_installed = vim.list_extend(vim.tbl_keys(servers), linters),
        })
