...
syntax match mumpsDotRegion /\s\+[. \t]*/ display contained
  \ nextgroup=mumpsCommand,mumpsComment
syntax match mumpsLabel /^[sub|def|fn][%A-Za-z][A-Za-z0-9]*:\?\|^[0-9]\+:\?/
...
highlight def link mumpsFormalLine mumpsLabelName
highlight def link mumpsCommentLine Normal

highlight def link mumpsDotLevel Normal
highlight def link mumpsDotRegion Normal
highlight def link mumpsLabel Error
...
highlight def link mumpsOperator Operator
highlight def link mumpsComma Operator
...
highlight def link mumpsFunctionList MumpsIntrinsicFunction

highlight def link mumpsCommand MumpsCommand
highlight def link mumpsFunction Error
highlight def link mumpsSpecialVariable Error
